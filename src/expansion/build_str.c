...
/*   Updated: 2023/10/18 18:23:26 by ncasteln         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*build_str(char *old_str, t_list *var_lst)
{
	...
	new_string = ft_calloc(total_len + 1, sizeof(char)); // protect
	...
	while (i < total_len)
	{
		...
		else // substitution
		{
			...
			else // if it's not the case of $ ------- // need to handle not-existent variables ???
			{
				ft_strlcpy(new_string + i, var->value, var->value_len + 1);
				i += var->value_len;
